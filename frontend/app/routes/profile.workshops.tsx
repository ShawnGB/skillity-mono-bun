...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const upcoming = workshops.filter(
    (w) => w.status === WorkshopStatus.DRAFT || w.status === WorkshopStatus.PUBLISHED,
  );
...
function WorkshopRow({
  ...
  return (
    <div className={cn("rounded-xl border bg-card p-5", dimmed && "opacity-60")}>
        ...
        <div className="min-w-0 flex-1">
          ...
          {workshop.pendingCount !== undefined && (
            <div className="flex items-center gap-4 mt-2 text-xs">
              <span className="text-green-600">{workshop.participantCount} confirmed</span>
              {(workshop.pendingCount ?? 0) > 0 && (
                <span className="text-yellow-600">{workshop.pendingCount} pending</span>
              )}
        ...
        <div className="space-y-4">
          {upcoming.map((workshop) => (
            <WorkshopRow
              key={workshop.id}
              workshop={workshop}
              dateCount={workshop.seriesId ? seriesCounts[workshop.seriesId] : undefined}
